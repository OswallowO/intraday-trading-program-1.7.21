# 1. 拉高進場和漲停進場同一時間觸發，沒有以漲停進場為優先。
# 2. 拉高進場和漲停進場有時無法設定領漲。
# 已完成：觸發漲停進場後，在接下來的等待期間加入「等待第 X 分鐘」的訊息、回測函數的相似度判斷、交易函數的相似度判斷、 領漲替換之後沒有繼續觀察、進場門檻改為0~6、修正50元以下股票抓不到停損價差問題、修正億點錯誤。
...
warnings.filterwarnings(
    ...
    message="ssl.match_hostname\\(\\) is deprecated"
)
    ...
    if pre_market_start <= now < market_start:
        if mode == 'full':
            try:
                ...
                print("等待時間必須是整數。")
                return
            hold_input = input("請輸入持有時間（分鐘，輸入 'F' 代表持有到13:30強制出場）：")
            hold_minutes = None if hold_input.upper() == 'F' else int(hold_input)
...
def calculate_group_kline_data(
    ...
    def df_of(sym):
        ...
        mask = (df['time'] >= start_str) & (df['time'] <= end_str)
    ...
    for sym in candidates:
        ...
        if df_lead.empty or df_cand.empty:
            ...
            if merged.empty:
                ...
                # … 標準化、計算 dist → sim = 1/(1+dist)
                for col in ['open','high','low','close','volume']:
                    merged[f'{col}_L_z'] = (merged[f'{col}_L'] - merged[f'{col}_L'].mean()) / merged[f'{col}_L'].std(ddof=0)
                    merged[f'{col}_C_z'] = (merged[f'{col}_C'] - merged[f'{col}_C'].mean()) / merged[f'{col}_C'].std(ddof=0)
                dist = np.sqrt(
                    (merged['open_L_z']   - merged['open_C_z'])**2 +
                    (merged['high_L_z']   - merged['high_C_z'])**2 +
                    (merged['low_L_z']    - merged['low_C_z'])**2 +
...
def filter_by_similarity_and_tracking(
    leader: str,
    group_symbols: list[str],
    tracking_list: list[str],
    intraday_file: str,
    wait_end_time: datetime,
    sim_threshold: float = .4,
    close_weight: float = 2.0,
) -> list[str]:
    """
    先對『整個群組』與 leader 做相似度比對，抓出 sim≥0.4 的股票，
    再和 tracking_list 取交集，最後回傳交集結果。
    """
    # ① 整個群組先跑一次
    sim_ok_group = calculate_realtime_kline_data(
        leader       = leader,
        candidates   = group_symbols,
    ...
    # ② 取交集
    return [s for s in sim_ok_group if s in tracking_list]
        ...
        if in_waiting_period:
            if waiting_time >= wait_minutes:
                ...
                for s, pk_time in passed_reversal:
                    ...
                    if rise_now is None or not (0 <= rise_now <= 6): continue
                ...
                for sym in filtered_stocks:
                    ...
                    if rise_now is None or not (0 <= rise_now <= 6):
                        continue
...
def process_live_trading_logic(
    ...
    for grp, gstat in group_positions.items():
        ...
        for sym in consolidated_symbols[grp]:
            ...
            # 加入條件：2min_pct_increase ≥ 1.5%
            if row_now["2min_pct_increase"] >= 1.5 and sym not in track:
                track[sym] = {
    ...
    for grp, gstat in group_positions.items():
        ...
        '''
        # 舊
        start_wait = gstat.get("wait_start", gstat["start_time"])
        '''
        # 新
        if "wait_start" not in gstat:
            continue
        start_wait = gstat["wait_start"]

        elapsed = (now_full - start_wait).total_seconds() / 60
        if elapsed >= wait_minutes - 1:
    ...
    for grp in groups_ready:
        ...
        # —— 2. 盤中相似度篩選 —— 
        # ⚠ 先對『整個群組』跑一次，再跟 tracking 取交集
        sim_ok = filter_by_similarity_and_tracking(
            leader        = leader_sym,
            group_symbols = consolidated_symbols[grp],
            tracking_list = list(track.keys()),
            intraday_file = "auto_intraday.json",
            wait_end_time = now_full,
            sim_threshold = 0.4,
            close_weight  = 2.0
        )
        print(f"[RT-SIM] 相似度合格且在追蹤清單：{sim_ok}")
        message_log.append((trading_txt, f"[RT-SIM] 相似度合格且在追蹤清單：{sim_ok}"))

        # ⇩ 用新的 sim_ok 取代原本的 filtered_track
        filtered_track = {s: track[s] for s in sim_ok}
        ...
        if chosen["symbol"] in stock_market_map.get("TSE", {}):
            contract = getattr(api.Contracts.Stocks.TSE, "TSE" + stock_code_str)
            market_label = "TSE"
        elif chosen["symbol"] in stock_market_map.get("OTC", {}):
            contract = getattr(api.Contracts.Stocks.OTC, "OTC" + stock_code_str)
            market_label = "OTC"
        else:
            print(f'⚠️ 找不到股票 {stock_code_str} 的市場資料！取消下單。')
            return
        ...
        if stock_code_str not in to.contracts:
            to.contracts[stock_code_str] = contract
        to.add_condition(tcond)

        msg = (
            f"{GREEN}進場！{stock_code_str}  {shares}張  "
